...
if __name__ == '__main__':
    ...
    fc_res = []
    for i in range(1,5):
        with open(f'./Base_Forecasts/Wiki/Wiki_deepar_e200_optuna_in_{i}.json','r') as file2:
            fc = json.load(file2)
            fc_res.append(fc)
    ...
    np.save('./Reconcile_and_Evaluation/Wiki/Wiki_deepar_Regularization_min_G_indep_optuna.npy',G_list[i])
    np.save('./Reconcile_and_Evaluation/Wiki/Wiki_deepar_Reluarization_loss_indep_optuna.npy',loss)
    np.save('./Reconcile_and_Evaluation/Wiki/Wiki_deepar_Regularization_G_indep_optuna.npy',G)

    x_axis = list(range(1,len(loss)+1))
    plt.plot(x_axis,loss,'-r')
    plt.title('Wiki::DeepAR 能量得分迭代图')
    plt.xlabel('迭代次数')
    plt.ylabel('能量得分')
    plt.savefig('./Plot_new/Wiki/Wiki_deepar_indep.png')
    plt.show()
