...
M<-366 #Number of series

# Read Smat
S<-fromJSON('./Data/Wiki/Wiki_Smat.json')
# S1<-fromJSON('.\\Reconcile_and_Evaluation\\Wiki\\tourism_s_json.json')
...
# Read base forecasts
fc1<-fromJSON('./Base_Forecasts/Wiki/Wiki_deepar_e100.json')
fc<-list()
for(i in 1:evalN){
...
write.csv(res_energyscore,'.\\Evaluation_Result_new\\Energy_Score\\Wiki_deepar_scaled.csv',row.names=FALSE)
write.csv(res_crps,'.\\Evaluation_Result_new\\CRPS\\Wiki_deepar_scaled.csv',row.names=FALSE)
write.csv(res_variogramscore,'.\\Evaluation_Result_new\\Variogram_Score\\Wiki_deepar_scaled.csv',row.names=FALSE)
