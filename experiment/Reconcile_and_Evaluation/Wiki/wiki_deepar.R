...
############# Set constant
evalN<-15 #Number of evaluation periods
Q<-1000 #Number of draws to estimate energy score
N<-351 #Training sample size
M<-199 #Number of series

# Read Smat
S<-fromJSON('./Data/Wiki/Wiki_Smat.json')
S1<-fromJSON('.\\Reconcile_and_Evaluation\\Wiki\\wiki_s_json.json')
G_opt <- npyLoad('./Reconcile_and_Evaluation/Wiki/Wiki_deepar_Regularization_G_indep_optuna.npy')
new_index<-c(2:199,1)
# Predefine
...
# Read raw data
cols_to_remove <- 1
data<-read.csv('./Data/Wiki/Wiki_process.csv')[,-cols_to_remove]

# Read base forecasts
fc1<-fromJSON('./Base_Forecasts/Wiki/Wiki_deepar_e200_optuna.json')
fc<-list()
for(i in 1:evalN){
...
write.csv(res_energyscore,'.\\Evaluation_Result_new\\Energy_Score\\Wiki_deepar.csv',row.names=FALSE)
write.csv(res_crps,'.\\Evaluation_Result_new\\CRPS\\Wiki_deepar.csv',row.names=FALSE)
write.csv(res_variogramscore,'.\\Evaluation_Result_new\\Variogram_Score\\Wiki_deepar.csv',row.names=FALSE)
