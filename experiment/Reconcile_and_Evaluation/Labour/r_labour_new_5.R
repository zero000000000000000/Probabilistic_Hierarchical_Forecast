...
############# Set constant
evalN<-6 #Number of evaluation periods
Q<-1000 #Number of draws to estimate energy score
N<-157 #Training sample size
M<-57 #Number of series

# Read Smat
S<-fromJSON('./Data/Labour/labour_Smat.json')
S1<-fromJSON('.\\Reconcile_and_Evaluation\\Labour\\labour_s_json_5.json')
G_opt <- npyLoad('./Reconcile_and_Evaluation/Labour/labour_ARIMA_Regularization_G_indep_optuna_5.npy')
new_index<-fromJSON('./Reconcile_and_Evaluation/Labour/labour_newindex_json_5.json')
# Predefine
SG_bu<-S%*%cbind(matrix(0,48,9),diag(rep(1,48)))
...
# Read base forecasts
fc<-fromJSON('./Reconcile_and_Evaluation/Labour/labour_out_process.json')
# fc<-fc[[1]]
for(i in 1:evalN){
...
for(i in 1:evalN){
  ...
  basis_lis<-forecast.basis_series(S,immu_set=c(10,13,14,15,50))
...
write.csv(res_energyscore,'.\\Evaluation_Result_new\\Energy_Score\\labour_ARIMA_5.csv',row.names=FALSE)
write.csv(res_crps,'.\\Evaluation_Result_new\\CRPS\\labour_ARIMA_5.csv',row.names=FALSE)
write.csv(res_variogramscore,'.\\Evaluation_Result_new\\Variogram_Score\\labour_ARIMA_5.csv',row.names=FALSE)
