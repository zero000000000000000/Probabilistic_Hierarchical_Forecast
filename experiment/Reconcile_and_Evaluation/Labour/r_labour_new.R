...
S1<-fromJSON('.\\Reconcile_and_Evaluation\\Labour\\labour_s_json.json')
G_opt <- npyLoad('./Reconcile_and_Evaluation/Labour/labour_G_mintshr.npy')
new_index<-c(2:57,1)
# Predefine
...
write.csv(res_energyscore,'.\\Evaluation_Result_new\\Energy_Score\\labour_ARIMA.csv',row.names=FALSE)
write.csv(res_crps,'.\\Evaluation_Result_new\\CRPS\\labour_ARIMA.csv',row.names=FALSE)
write.csv(res_variogramscore,'.\\Evaluation_Result_new\\Variogram_Score\\labour_ARIMA.csv',row.names=FALSE)

# > ans<-SG_MinTShr[,c(2:57,1)]
# > View(ans)
# > ans<-as.data.frame(ans)
# > ans<-SG_MinTShr[,c(2:57,1)]
# > json_string <- toJSON(ans, pretty = TRUE)
# > write(json_string, "./Reconcile_and_Evaluation/Labour/G_MinTShr.json")
