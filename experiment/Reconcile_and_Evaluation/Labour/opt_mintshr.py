...
if __name__ == '__main__':
    ...
    #G = np.zeros((m1,n))
    with open('.\\Reconcile_and_Evaluation\\Labour\\G_MinTShr.json') as f:
        G = json.load(f)
    G = np.array(G)
    G[(m1-1),(n-1)] = 1
    ...
    np.save('./Reconcile_and_Evaluation/Labour/labour_min_G_mintshr.npy',G_list[ind])
    np.save('./Reconcile_and_Evaluation/Labour/labour_loss_mintshr.npy',loss)
    np.save('./Reconcile_and_Evaluation/Labour/labour_G_mintshr.npy',G)
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('能量得分')
    plt.savefig('./Plot_new/Labour/labour_mintshr.png')
    plt.show()