...
def loss_function(G):
    ...
    l1_lambda = 100
    l2_lambda = 100
    l1_regularization = l1_lambda*np.sum(np.abs(G[:47,:]))
    l2_regularization = l2_lambda*np.sum(np.square(G[:47,:]))
...
if __name__ == '__main__':
    ...
    with open('./Reconcile_and_Evaluation/Labour/labour_tranformed_Smat.json','r') as file:
        S = json.load(file)
    S = np.array(eval(S[0]))

    new_index = [i for i in range(1,57)]+[0]
    ...
    np.save('./Reconcile_and_Evaluation/Labour/labour_ARIMA_Regularization_min_G_indep_optuna_v1.npy',G_list[ind])
    np.save('./Reconcile_and_Evaluation/Labour/labour_ARIMA_Reluarization_loss_indep_optuna_v1.npy',loss)
    np.save('./Reconcile_and_Evaluation/Labour/labour_ARIMA_Regularization_G_indep_optuna_v1.npy',G)
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('能量得分')
    plt.savefig('./Plot_new/Labour/labour_ARIMA_indep_v1.png')
    plt.show()