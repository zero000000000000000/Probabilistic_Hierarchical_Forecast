...
directory_path<-'./Evaluation_Result/Variogram_Score'
if (!dir.exists(directory_path)){
  dir.create(directory_path, recursive = TRUE)
  print(paste("Path created:", directory_path))
}

write.csv(res_energyscore,'.\\Evaluation_Result\\Energy_Score\\Tourism.csv',row.names=FALSE)
write.csv(res_crps,'.\\Evaluation_Result\\CRPS\\Tourism.csv',row.names=FALSE)
write.csv(res_variogramscore,'.\\Evaluation_Result\\Variogram_Score\\Tourism.csv',row.names=FALSE)
