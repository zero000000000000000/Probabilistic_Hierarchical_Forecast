...
S1<-fromJSON('.\\Reconcile_and_Evaluation\\Tourism\\tourism_s_json_5.json')
G_opt <- npyLoad('./Reconcile_and_Evaluation/Tourism/Tourism_deepar_Regularization_G_indep_optuna_5.npy')
new_index<-fromJSON('.\\Reconcile_and_Evaluation\\Tourism\\Tourism_tranformed_newindex_5.json')
# Predefine
...
for(i in 1:evalN){
  ...
  basis_lis<-forecast.basis_series(S,immu_set=c(12, 23, 50, 64, 68))
...
write.csv(res_energyscore,'.\\Evaluation_Result_new\\Energy_Score\\Tourism_deepar_5.csv',row.names=FALSE)
write.csv(res_crps,'.\\Evaluation_Result_new\\CRPS\\Tourism_deepar_5.csv',row.names=FALSE)
write.csv(res_variogramscore,'.\\Evaluation_Result_new\\Variogram_Score\\Tourism_deepar_5.csv',row.names=FALSE)
