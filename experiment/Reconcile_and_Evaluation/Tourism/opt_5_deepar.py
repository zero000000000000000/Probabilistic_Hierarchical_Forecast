...
def loss_function(G):
    ...
    l1_lambda = 1000
    l2_lambda = 1000
    l1_regularization = l1_lambda*np.sum(np.abs(G[:71,:]))
    l2_regularization = l2_lambda*np.sum(np.square(G[:71,:]))
...
if __name__ == '__main__':
    ...
    with open('./Reconcile_and_Evaluation/Tourism/Tourism_tranformed_newindex_5.json','r') as file0:
    ...
    fc_res = []
    for i in range(1,5):
        with open(f'./Base_Forecasts/Tourism/Tourism_deepar_e300_optuna_in_{i}.json','r') as file2:
            fc = json.load(file2)
            fc_res.append(fc)
    ...
    np.save('./Reconcile_and_Evaluation/Tourism/Tourism_deepar_Regularization_min_G_indep_optuna_5.npy',G_list[ind])
    np.save('./Reconcile_and_Evaluation/Tourism/Tourism_deepar_Reluarization_loss_indep_optuna_5.npy',loss)
    np.save('./Reconcile_and_Evaluation/Tourism/Tourism_deepar_Regularization_G_indep_optuna_5.npy',G)

    x_axis = list(range(1,len(loss)+1))
    plt.plot(x_axis,loss,'-r')
    plt.title('Tourism::DeepAR 能量得分迭代图')
    plt.xlabel('迭代次数')
    plt.ylabel('能量得分')
    plt.savefig('./Plot_new/Tourism/Tourism_deepar_indep_5.png')
    plt.show()