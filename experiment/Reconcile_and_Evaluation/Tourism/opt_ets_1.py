...
def loss_function(G):
    ...
    l1_lambda = 1000
    l2_lambda = 1000
    l1_regularization = l1_lambda*np.sum(np.abs(G[:75,:]))
    l2_regularization = l2_lambda*np.sum(np.square(G[:75,:]))
...
if __name__ == '__main__':
    ...
    for iter_time in range(1):
        ...
        # np.save(f'./Reconcile_and_Evaluation/Tourism/src/Tourism_ETS_1_{iter_time}.npy',G_list[ind])
        # np.save(f'./Reconcile_and_Evaluation/Tourism/src/Tourism_ETS_1_{iter_time}.npy',loss)
        np.save(f'./Reconcile_and_Evaluation/Tourism/src/Tourism_ETS_1_{iter_time}.npy',G)
